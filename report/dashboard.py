...
class LineChart(MatplotlibViz):
    ...
    def visualization(self, asset_id, model):
        ...
        ax.set_title('Cumulative Events', fontsize=11, pad=15)
        ax.set_xlabel('Date')
        ax.set_ylabel('Count')
...
class BarChart(MatplotlibViz):
    ...
    def visualization(self, asset_id, model):
        ...
        if data.empty:
            fig, ax = plt.subplots()
            ax.text(0.5, 0.5, "No data to score recruitment risk", ha="center", va="center")
            ax.axis("off")
            return
        
        # Using the predictor class attribute
        # pass the data to the `predict_proba` method
        pred_probs = self.predictor.predict_proba(data)
